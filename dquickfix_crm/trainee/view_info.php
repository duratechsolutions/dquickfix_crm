...
$query_Recordset1 = "SELECT T2.*, DATE_FORMAT(DATE_ADD(T2.`date`, INTERVAL 7 DAY), '%Y-%m-%d') AS InsertDate FROM issue_info AS T2 LEFT JOIN computer_details AS T1 ON T2.computer_details_id = T1.id WHERE T2.customer_id = '$colname_Recordset1' ORDER BY T2.id ASC";
